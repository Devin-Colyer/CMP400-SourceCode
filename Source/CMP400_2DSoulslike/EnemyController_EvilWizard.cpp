...
AEnemyController_EvilWizard::AEnemyController_EvilWizard(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	...
	AIPerceptionComp->OnTargetPerceptionUpdated.AddDynamic(this, &AEnemyController_EvilWizard::OnTargetPerceptionUpdated);

	/*GetActionsComp();
	GetPathFollowingComponent();*/
}

void AEnemyController_EvilWizard::OnPossess(APawn* InPawn)
{
	Super::OnPossess(InPawn);

	AEnemy_EvilWizard* Self = Cast<AEnemy_EvilWizard>(InPawn);
	if (Self) {
...
void AEnemyController_EvilWizard::OnTargetPerceptionUpdated(AActor* Actor, FAIStimulus Stimulus)
{
